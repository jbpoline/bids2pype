...
TESTFILESDIR = osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'test_files')
...
def test_get_nipype_specify_model_inputs(): 
    ...
    exp_param_gain = np.loadtxt(osp.join(TESTFILESDIR,'./test_cond_amplitude.txt')).astype(float)
...
def test_get_other_regressors():
    ...
    assert sorted_data[0] == base_dir + '/sub-01/func/sub-01_task-mixedgamblestask_run-01_bold.nii.gz'
    assert sorted_data[2] == base_dir + '/sub-01/func/sub-01_task-mixedgamblestask_run-03_bold.nii.gz'
    assert sorted_data[-1] == base_dir + '/sub-16/func/sub-16_task-mixedgamblestask_run-03_bold.nii.gz'
